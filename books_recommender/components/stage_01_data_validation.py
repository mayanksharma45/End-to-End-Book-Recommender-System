...
class DataValidation:
    ...
    def preprocess_data(self):
        try:
            ...
            # index=False means: Tells pandas not to write row numbers (index) in the CSV file.
            ...
            pickle.dump(final_rating,open(os.path.join(self.data_validation_config.serialized_objects_dir, "final_rating.pkl"),'wb')) # 'wb' means "write in binary" — required for saving with pickle
            # Save the final_rating DataFrame as a pickle file named final_rating.pkl in the serialized_objects_dir folder so it can be quickly loaded later without reprocessing
