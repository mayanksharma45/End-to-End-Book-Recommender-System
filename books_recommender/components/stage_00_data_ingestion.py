...
class DataIngestion:
    ...
    def download_data(self):
        ...
        try:
            ...
            data_file_name = os.path.basename(dataset_url)    # if the URL is https://domain.com/data.zip, this line returns "data.zip"
            zip_file_path = os.path.join(zip_download_dir, data_file_name)
            logging.info(f"Downloading data from {dataset_url} into file {zip_file_path}")
            urllib.request.urlretrieve(dataset_url,zip_file_path)  # This downloads the file from the dataset_url and saves it at zip_file_path
            logging.info(f"Downloaded data from {dataset_url} into file {zip_file_path}")
            return zip_file_path   # unzipping it later.
